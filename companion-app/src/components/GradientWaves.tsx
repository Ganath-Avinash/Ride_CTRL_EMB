...
// Detect low-end mobile: limit to 'low' detail and DPR=1
const isMobileLowEnd = (): boolean => {
  if (typeof navigator === 'undefined') return false;
  const cores = (navigator as any).hardwareConcurrency ?? 4;
  const dpr = window.devicePixelRatio ?? 1;
  // Mobile with <=4 cores or DPR exactly 1 (budget device) → low-end
  return cores <= 4 || dpr <= 1;
};
const MOBILE_DPR_CAP = isMobileLowEnd() ? 1 : Math.min(window.devicePixelRatio || 1, 1.5);

const detailToSteps = (d: GradientWavesDetail) =>
  isMobileLowEnd()
    ? (d === 'high' ? 50 : d === 'medium' ? 35 : 25) // capped for mobile
    : (d === 'low' ? 40 : d === 'high' ? 110 : 70);
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  swell = 35, turbulence = 20, tilt = 1.11, zoom = 1.0,
  height = 5.5, fogDepth = 15, detail = 'medium',
  brightness = 1.0, opacity = 1.0, mouseInteraction = true,
  parallaxStrength = 0.5, grain = true, grainIntensity = 0.05, className = '',
}) => {
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({
        ...
        antialias: false,       // antialias off → big GPU saving
        dpr: MOBILE_DPR_CAP,   // cap to 1 on mobile
      });
      gl = renderer.gl;
    ...
    const program = new Program(gl, {
      vertex, fragment, uniforms: {
        ...
        uFogDepth: { value: 15 }, uSteps: { value: detailToSteps(detail) },
        uBrightness: { value: 1.0 }, uOpacity: { value: 1.0 },
        uGrain: { value: 1.0 }, uGrainIntensity: { value: 0.05 },
        uMouse: { value: new Float32Array([0.5, 0.5]) },
        uParallax: { value: 0.5 }, uEnableMouse: { value: true },
    ...
    const setSize = () => {
      ...
      (program.uniforms.iResolution as { value: Float32Array }).value[0] = gl.drawingBufferWidth;
      (program.uniforms.iResolution as { value: Float32Array }).value[1] = gl.drawingBufferHeight;
    ...
    const cMouse: [number, number] = [0.5, 0.5];
    const tMouse: [number, number] = [0.5, 0.5];
    const onPM = (e: PointerEvent) => {
      const r = canvas.getBoundingClientRect();
      tMouse[0] = (e.clientX - r.left) / r.width;
      tMouse[1] = 1.0 - (e.clientY - r.top) / r.height;
    };
    const onPL = () => { tMouse[0] = 0.5; tMouse[1] = 0.5; };
    canvas.addEventListener('pointermove', onPM);
    canvas.addEventListener('pointerleave', onPL);
    ...
    const loop = (t: number) => {
      ...
      const tx = enableMouseRef.current ? tMouse[0] : 0.5;
      const ty = enableMouseRef.current ? tMouse[1] : 0.5;
      cMouse[0] += 0.05 * (tx - cMouse[0]);
      cMouse[1] += 0.05 * (ty - cMouse[1]);
      const m = (program.uniforms.uMouse as { value: Float32Array }).value;
      m[0] = cMouse[0]; m[1] = cMouse[1];
      renderer.render({ scene: mesh });
    ...
    return () => {
      ...
      canvas.removeEventListener('pointerleave', onPL);
