...
// ================================================================
const RIDE_CTRL_SERVICE_UUID = '4fafc201-1fb5-459e-8fcc-c5c9c331914b';
...
class BleService {
  ...
  async connect(): Promise<'connected' | 'failed' | 'unsupported'> {
    ...
    try {
      this.device = await nav.bluetooth.requestDevice({
        filters: [{ name: 'RIDE_CTRL' }],
        optionalServices: [RIDE_CTRL_SERVICE_UUID],
      });
      ...
      const service = await this.server.getPrimaryService(RIDE_CTRL_SERVICE_UUID);
  ...
  get deviceName(): string {
    return (this.device?.name as string) ?? 'RIDE_CTRL';
  }
}
